...
h264_1080p(){
  /usr/lib/jellyfin-ffmpeg/ffmpeg -y -hide_banner -benchmark -report -c:v h264_qsv -i /config/ribblehead_1080p_h264.mp4 -c:a copy -c:v hevc_qsv -preset slow -global_quality 18 -look_ahead 1 -f null - 2>/dev/null
}

h264_4k(){
  /usr/lib/jellyfin-ffmpeg/ffmpeg -y -hide_banner -benchmark -report -c:v h264_qsv -i /config/ribblehead_4k_h264.mp4 -c:a copy -c:v hevc_qsv -preset slow -global_quality 18 -look_ahead 1 -f null - 2>/dev/null
}
